...
const Typhoon = artifacts.require('./BNBTyphoon.sol');
...
contract('BNBTyphoon', (accounts) => {
  ...
  before(async () => {
    ...
    typhoon = await Typhoon.deployed();
  ...
  describe('#constructor', () => {
    it('should initialize', async () => {
      const etherDenomination = await typhoon.denomination();
  ...
  describe('#deposit', () => {
    it('should emit event', async () => {
      ...
      let { logs } = await typhoon.deposit(commitment, { value, from: sender });
      ...
      ({ logs } = await typhoon.deposit(commitment, { value, from: accounts[2] }));
    ...
    it('should throw if there is a such commitment', async () => {
      ...
      await typhoon.deposit(commitment, { value, from: sender }).should.be.fulfilled;
      const error = await typhoon.deposit(commitment, { value, from: sender }).should.be.rejected;
  ...
  describe('#withdraw', () => {
    it('should work', async () => {
      ...
      // let gas = await typhoon.deposit.estimateGas(toBN(deposit.commitment.toString()), { value, from: user, gasPrice: '0' })
      // console.log('deposit gas:', gas)
      await typhoon.deposit(toFixedHex(deposit.commitment), { value, from: user, gasPrice: '0' });
      ...
      const balanceTyphoonBefore = await web3.eth.getBalance(typhoon.address);
      ...
      let isSpent = await typhoon.isSpent(toFixedHex(input.nullifierHash));
      isSpent.should.be.equal(false);

      // Uncomment to measure gas usage
      // gas = await typhoon.withdraw.estimateGas(proof, publicSignals, { from: relayer, gasPrice: '0' })
      ...
      const { logs } = await typhoon.withdraw(proof, ...args, { from: relayer, gasPrice: '0' });

      const balanceTyphoonAfter = await web3.eth.getBalance(typhoon.address);
      ...
      balanceTyphoonAfter.should.be.eq.BN(toBN(balanceTyphoonBefore).sub(toBN(value)));
      ...
      isSpent = await typhoon.isSpent(toFixedHex(input.nullifierHash));
    ...
    it('should prevent double spend', async () => {
      const deposit = generateDeposit();
      await tree.insert(deposit.commitment);
      await typhoon.deposit(toFixedHex(deposit.commitment), { value, from: sender });
      ...
      await typhoon.withdraw(proof, ...args, { from: relayer }).should.be.fulfilled;
      const error = await typhoon.withdraw(proof, ...args, { from: relayer }).should.be.rejected;
    ...
    it('should reject with tampered public inputs', async () => {
      ...
      let error = await typhoon.withdraw(proof, ...incorrectArgs, { from: relayer }).should.be.rejected;
      ...
      error = await typhoon.withdraw(proof, ...incorrectArgs, { from: relayer }).should.be.rejected;
      error.reason.should.be.equal('Invalid withdraw proof');
      ...
      await typhoon.withdraw(proof, ...args, { from: relayer }).should.be.rejected;

      // should work with original values
      await typhoon.withdraw(originalProof, ...args, { from: relayer }).should.be.fulfilled;
    });
  ...
  describe('#changeOperator', () => {
    it('should work', async () => {
      let operator = await typhoon.operator();
      operator.should.be.equal(sender);

      const newOperator = accounts[7];
      await typhoon.changeOperator(newOperator).should.be.fulfilled;

      operator = await typhoon.operator();
    ...
    it('cannot change from different address', async () => {
      ...
      const newOperator = accounts[7];
      const error = await typhoon.changeOperator(newOperator, { from: accounts[7] }).should.be.rejected;
  ...
  describe('#updateVerifier', () => {
    it('should work', async () => {
      ...
      await typhoon.updateVerifier(newVerifier).should.be.fulfilled;

      const verifier = await typhoon.verifier();
    ...
    it('cannot change from different address', async () => {
      ...
      const newVerifier = accounts[7];
      const error = await typhoon.updateVerifier(newVerifier, { from: accounts[7] }).should.be.rejected;
      error.reason.should.be.equal('Only operator can call this function.');
    });
  });
  ...
  describe('#isSpent', () => {
    it('should work', async () => {
      ...
      await typhoon.deposit(toFixedHex(deposit1.commitment), { value, gasPrice: '0' });
      await typhoon.deposit(toFixedHex(deposit2.commitment), { value, gasPrice: '0' });
      ...
      await typhoon.withdraw(proof, ...args, { from: relayer, gasPrice: '0' });
      ...
      const spentArray = await typhoon.isSpentArray([nullifierHash1, nullifierHash2]);
